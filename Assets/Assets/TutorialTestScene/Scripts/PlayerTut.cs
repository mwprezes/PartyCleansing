...
public class PlayerTut : MonoBehaviour {
        ...
        if (isHolding)
        {
            if (Input.GetKeyDown(KeyCode.F) /*|| Input.GetMouseButtonDown(0)*/)
            {
                if (storage != null && (!storageFull || !storageTrapped))
                {
                    ...
                    HintText = "Wciśnij 'F' aby wyjąć przedmiot";
                    StartCoroutine(Wait());
    ...
    void OnGUI()
    {
        ...
        if (HintShow)
        {
            ...
            GUI.Label(new Rect(HintPosition.x - 20, Screen.height - HintPosition.y - 70, 250, 25), "<size=20>" + HintText + "</size>");
        }
    ...
    private void OnTriggerEnter(Collider hit)
    {
        if (hit.gameObject.tag == "Pickable")
        {
            ...
            HintText = "Wciśnij 'F', aby podnieść";
            StartCoroutine(Wait());
        ...
        if (hit.gameObject.tag == "Distract")
        {
            ...
            HintText = "Wciśnij 'C', aby włączyć radio";
            StartCoroutine(Wait());
        ...
        else if (hit.gameObject.tag == "Combine" && isHolding)
        {
            ...
            HintText = " Wciśnij 'F' by połączyć";
            StartCoroutine(Wait());
        }
    }
